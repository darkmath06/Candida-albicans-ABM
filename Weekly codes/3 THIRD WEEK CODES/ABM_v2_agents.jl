...
const ANTIFUNGAL_DAMAGE_THRESHOLD = 0.5      # Lowered threshold so damage starts accumulating earlier (based on graph)
...
# --- Stress Recovery (PCD+ ONLY) ---
const APOPTOSIS_RECOVERY_PROB = 0.15         # Probability to reverse early apoptosis per hour
const RETAIN_DIVISION_PROB = 0.0             # Probability a resuscitated cell can still replicate
const STRESS_CLEARANCE_HALF_LIFE = 2.0       # Hours required to clear 50% of stress memory
...
has_recovered(a::PCDPlusCell) = a.has_recovered
has_recovered(a::PCDMinusCell) = false
...
function initialize_model(AgentType::Type)
    ...
    for _ in 1:INITIAL_CELLS
        if AgentType === PCDPlusCell
            add_agent_single!(PCDPlusCell, model, true, false, 0.0, 0.0, false, false, false, 0.0, 1.0, true, false)
        else
...
function get_death_rates(c::Float64)
    ...
    # 1. Calculate the final cumulative percentages we WANT to see overnight
    raw_apop_frac = 0.60 * exp(- (log(c / 8.0))^2 / 0.4)
    raw_necro_frac = 0.95 / (1.0 + exp(-0.35 * (c - 9.0)))
    
    base_apop_frac = 0.60 * exp(- (log(2.0 / 8.0))^2 / 0.4)
    base_necro_frac = 0.95 / (1.0 + exp(-0.35 * (2.0 - 9.0)))
    
    target_apop_frac = max(0.0, raw_apop_frac - base_apop_frac)
    target_necro_frac = max(0.0, raw_necro_frac - base_necro_frac)
    ...
    # 2. Convert the cumulative overnight fraction into a continuous HOURLY rate
    # Using exponential decay: S(t) = exp(-rate * t) -> S(24) = 1 - target_total_frac
    hourly_total_rate = -log(1.0 - target_total_frac) / ASSAY_DURATION_HOURS
    
    # 3. Split the hourly rate back into apoptosis and necrosis components
...
function apply_stress!(agent::PCDPlusCell, local_ANTIFUNGAL::Float64, model)
    ...
    if local_ANTIFUNGAL >= ANTIFUNGAL_DAMAGE_THRESHOLD
        ...
        if agent.ANTIFUNGAL_exposure_time < 0.01; agent.ANTIFUNGAL_exposure_time = 0.0; end
    end
...
function apply_stress!(agent::PCDMinusCell, local_ANTIFUNGAL::Float64, model)
    ...
    if agent.ANTIFUNGAL_exposure_time >= STRESS_START_TIME
        rate_apop, rate_necro = get_death_rates(local_ANTIFUNGAL)
        # PCD- cells suffer the exact same overall mortality rate, but exclusively as necrosis
        total_rate = rate_apop + rate_necro
    ...
    for agent in allagents(model)
        ...
        if agent.alive && get(n_demands, agent.id, 0.0) > 0
            ...
            if !is_apoptotic(agent)
                ...
                # If it cannot afford maintenance, it just stops growing (no endogenous shrinkage).
    ...
    for (pos, b) in newborn_spots
        if isempty(pos, model)
            if model.is_pcd_plus
                add_agent!(pos, PCDPlusCell, model, true, false, 0.0, 0.0, false, false, false, 0.0, b, true, false)
            else
...
function generate_subplots(model, title_prefix::String, step::Int)
    ...
    healthy_x = Float64[a.pos[1] for a in alive_agents if !is_apoptotic(a) && !has_recovered(a)]
    healthy_y = Float64[a.pos[2] for a in alive_agents if !is_apoptotic(a) && !has_recovered(a)]
    
    recovered_x = Float64[a.pos[1] for a in alive_agents if has_recovered(a)]
    recovered_y = Float64[a.pos[2] for a in alive_agents if has_recovered(a)]
...
function main()
    println("Initializing strictly typed Agents.jl models...")
