...
const ANTIFUNGAL_INJECTION_STEP = 1 # 24h
...
const ANTIFUNGAL_SOURCE_DOSE = 10000 # High concentration placed at each source pixel to diffuse outwards
const ANTIFUNGAL_SOURCE_RADIUS = 2 # If using POINT_SOURCES, how many pixels around the source get the initial dose (to create a small cluster instead of a single point)
...
const INIT_ANTIFUNGAL_LEVEL = 5 # Used if SPATIAL_MODE is UNIFORM
...
const ANTIFUNGAL_DAMAGE_THRESHOLD = 3 #0.64     # Threshold to start accumulating damage (x-intercept)
